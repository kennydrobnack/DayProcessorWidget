...
function stopDisplayUpdateTimer()
{
    //do nothing. We don't want the timer to ever stop
    //if (updateTimerDisplayInterval) {
...
function getMidnightTonight() {
    ...
    var tomorrow = new Date(now.getTime() + millisecondsPerDay);
    var midnightTonight = new Date(tomorrow.getFullYear(),
        tomorrow.getMonth(),
        tomorrow.getDate(),
        0, 0, 0, 0);
...
//Call at the end of the day when countdown reaches 0. 
//  resets the timer & starts it all over again for the day.
function resetTimer() {
    isTargetReached = false;
    attributes.targetDateTime = getMidnightTonight();
    attributes.targetLabel = "End of " + getTodaysDate();
    setCountdownTarget({ eventTime: attributes.targetDateTime,
                         eventLabel: attributes.targetLabel });
...
function load()
{
    ...
    //Set the eventLabel, eventTime, and start the timer now
    resetTimer();
}
